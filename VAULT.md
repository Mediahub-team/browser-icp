...
>
> **Единственное исключение — межитерационная петля 7→0:** офферы (этап 7) предлагают новые
> возможности в функциональность **следующей** версии (`version/<version-name>`, [[BRANCHING]] §4).
> Это не цикл в графе — внутри одной итерации DAG ацикличен, связь идёт в **новую** версию файла.

**При переисполнении этапа** старые артефакты и устаревший downstream **удаляются**, а ссылки
на них — репойнтятся на новые (порядок и проверка `check-vault.py` — [[BRANCHING]] §3
«Переисполнение этапа»). Висячих `[[ссылок]]` в ваулте не держим: граф не должен двоиться
